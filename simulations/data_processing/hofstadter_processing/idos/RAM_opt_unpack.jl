...
function unpack_and_slice_by_slope(
   ...
   println("Unpacking Hofstadter data from folder: ", folder_path_hof)
    ...
    # UPDATED: Key now includes phi instead of phason
    combo_to_files = Dict{Tuple{Float64, Tuple{Vararg{Float64}}, Float64, Float64}, String}()
    println("Caching base slices grouped by (Delta, t_n, phi, mu)...")
    
    # GROUP BY PHI (SLOPE) INSTEAD OF PHASON
    grouped = DataFrames.groupby(df_base, [:Delta, :t_n, :phi, :mu])
    total_groups = length(grouped)
    ...
    @showprogress for (i, g) in enumerate(grouped)
        ...
        t_n_vec = slice_df.t_n[1]
        t_n_tuple = Tuple(t_n_vec)
        phi_val = slice_df.phi[1] # Extract phi

        bytes = Base.summarysize(slice_df)
        row_cnt = nrow(slice_df)
        # Check eigen column existence before length (defensive)
        eig_len = isempty(slice_df.eigenvalues) ? 0 : length(slice_df.eigenvalues[1])

        # UPDATED FILENAME: phi instead of phason
        fname = @sprintf("slice_Delta%.5f_t1%.5f_t2%.5f_phi%.5f_mu%.5f.bson",
                         Delta, t_n_tuple[1], t_n_tuple[2], phi_val, mu_val)
        fpath = joinpath(base_slice_dir, fname)
        BSON.bson(fpath, Dict(:df_slice => slice_df))
        
        # Store with phi in key
        combo_to_files[(Delta, t_n_tuple, phi_val, mu_val)] = fpath
    end
