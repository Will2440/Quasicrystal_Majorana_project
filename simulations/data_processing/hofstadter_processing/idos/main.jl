...
df = compute_eigs_norm!(df; eigs_col=:eigenvalues, out_col=:eigs_norm)
df = compute_eigs_inner_norm!(df; eigs_col=:eigenvalues, out_col=:eigs_inner_norm)
...
phason_range = sort(unique(df.phason))
