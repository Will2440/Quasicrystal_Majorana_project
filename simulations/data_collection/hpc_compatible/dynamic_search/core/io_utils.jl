...
function split_points_into_chunks(points_csv::String, out_dir::String, chunk_size::Int)
    df = CSV.read(points_csv, DataFrame)
    n = nrow(df)
    n == 0 && return String[]
    ...
    for i in 1:n_chunks
        a = (i - 1) * chunk_size + 1
        b = min(i * chunk_size, n)
        sub = df[a:b, :]
        p = joinpath(out_dir, "points_chunk_$(lpad(i, 4, '0')).csv")
        CSV.write(p, sub)
        push!(chunk_paths, p)
    end
...
function read_worker_results(results_glob_dir::String)
    files = filter(f -> endswith(f, ".csv") && occursin("worker_results_", f), readdir(results_glob_dir; join=true))
    if isempty(files)
        return DataFrame(
